...
exports.getNewsSettings = async (req, res) => {
  try {
    let settings = await NewsSettings.findOne();
    if (!settings) {
      settings = await NewsSettings.create({
        hero: {
...
exports.updateNewsSettings = async (req, res) => {
  try {
    let settings = await NewsSettings.findOne();
    if (settings) {
      ...
      const newSettings = await NewsSettings.create(req.body);
